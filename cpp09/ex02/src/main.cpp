...
void printVecOfVec(vector<vector<int> > vecOfVec) {
  std::cout << "[";
  for (vector<vector<int> >::iterator vIt = vecOfVec.begin();
...
std::vector<int> getJacobsthalInsertionOrder(int n) {
  ...
  for (vector<int>::reverse_iterator it = jacobSeq.rbegin();
       it != jacobSeq.rend(); ++it) {
    if (*it < n)
      order.push_back(*it);
  }
...
void mergeSort(vector<int> &main) {
  ...
  vector<int> newMain;
  vector<int> newPend;
  for (vector<vector<int> >::iterator it = pairs.begin(); it != pairs.end();
       ++it) {
  ...
  for (vector<int>::iterator it = jacobSeq.begin(); it != jacobSeq.end();
       ++it) {
